...
export default function codesRegion(countryShortCode: string, regionShortCode: string) {
    let result: any
    CONFIG_CODES_REGION.map(item => {
        if(item.countryShortCode === countryShortCode) {
            result = item.regions.find(elem => elem.shortCode === regionShortCode)
        }
    })
...
// export default CONFIG_CODES_REGION