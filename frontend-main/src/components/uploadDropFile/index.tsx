...
export interface UploadDropFile_File extends File {
    _splice?: boolean
}
...
export default function UploadDropFile({
    ...
    maxFiles = 1,
    maxFileSize = CONFIG.maxFileSizeToUpload,
    ...
    if(filesLoaded > 0 && !id) throw new Error("[uploadDropFile] Для filesLoaded не указан ID")
        ...
        if(onLoad) {
            ...
            if(types) {
                acceptedFiles.map((item: UploadDropFile_File, i) => {
                    ...
                    if(item.size >= maxFileSize) acceptedFiles.splice(i, 1)
                })
