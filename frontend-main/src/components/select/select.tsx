...
export function Select({
    ...
    const [ toggle, setToggle ] = React.useState(autoOpened || false)
    React.useEffect(() => {
        onToggle?.(toggle)
    }, [toggle])
    ...
    function onSearch(text: string) {
        ...
        let hideCount = 0
        list.map((item, i) => {
            if((version !== 2 && item[0].toLowerCase().indexOf(text) === -1 && item[1].toLowerCase().indexOf(text) === -1)
                || (version === 2 && item.key.toLowerCase().indexOf(text) === -1 && item.content.toLowerCase().indexOf(text) === -1)) {
                hideCount ++
    ...
    return (
        <div id={id} className={`select ${disabled && 'disabled'} ${classes}`} style={{...style}} ref={selectRef} data-toggle={toggle}>
            {title ? (<h5 className="selectTitle">{title}</h5>) : ''}
