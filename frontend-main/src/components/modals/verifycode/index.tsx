...
export default function ModalVerifyCode({
    ...
    React.useEffect(() => {
        if(code.length === length) {
            ...
            API({
                ...
                if(result.statusCode === 200) {
                    let verifycodes: string[] = new CustomStorage().get('verifycodes')
                    if(!verifycodes) verifycodes = []

                    verifycodes.push(result.message)
                    new CustomStorage().set('verifycodes', verifycodes)
    ...
    return (
        <Modal toggle={true} title={Language("VERIFYCODE_TITLE")} desciption={description}
