...
interface PreviewFilesContextListElement extends ContextMenuListElement {
    onSubmit?(file: PreviewFilesFile | StorageDTO, index: number): void
}

interface PreviewFilesProps {
    files: Array<PreviewFilesFile> | Array<StorageDTO>,
    context?: Array<PreviewFilesContextListElement>,

    hideDeleteBtn?: boolean,

    onDeleteFile?(file: PreviewFilesFile | StorageDTO, index: number): void
}
export default function PreviewFiles({
    ...
    function DropDownMenuRender({ index, file }: { index: number, file: PreviewFilesFile | StorageDTO }) {
        let fileURL

        if((file as StorageDTO).id) fileURL = (file as StorageDTO).type === 'image' ? (file as StorageDTO).images[0].link : ''
        ...
        if(!hideDeleteBtn) list.push({ content: Language("PREVIEWFILES_IMAGE_CONTEXT_DELETE", null, null, file.type.indexOf('image/') !== -1 ? Language("IMAGE") : Language("FILE")), color: 'var(--tm-color-red)', bottom: true, onClick: () => {
    ...
    return (
        <div className="previewfiles">
            {files.map((item: PreviewFilesFile | StorageDTO, i) => {
                let image: string

                if((item as StorageDTO).id) image = (item as StorageDTO).type === 'image' ? (item as StorageDTO).images[0].link : ''
                ...
                return (
                    <div key={i} className={`previewfiles-file ${(item as PreviewFilesFile).className}`}>
                        ...
                        {item.type.indexOf('image/') !== -1 || item.type === 'image' ?
