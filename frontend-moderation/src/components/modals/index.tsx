...
export function Modal({
    ...
    return (
        <div id={id} className={`modalindex ${classes} ${toggle && 'show'}`} style={{display: !toggle ? 'none' : 'flex'}} onClick={event => {
            ...
            <div className={`modalWrapper ${(phoneHideBtn && window.isPhone) ? 'phoneHideBtn' : ''}`} style={style} ref={elementRef}>
                ...
                {(body && body.map) ? (
                    ...
                    !body ? '' : (
                        <div style={{ overflow: modalBodyOverflow }} className="modalindexbody">{body}</div> 
                    )
                )}
                {navButtons ? 
