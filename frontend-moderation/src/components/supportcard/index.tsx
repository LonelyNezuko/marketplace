...
export default function SupportCard({
    ...
    const [ report, setReport ] = React.useState<ModerationSupportDTO>(null)
    ...
    React.useMemo(() => {
        ...
        Gateway.on('moderation', 'onSupportMessage', (message: ModerationSupportMessageDTO) => {
            setMessages(old => {
                old.push(message)
                return [...old]
            })
        })
        Gateway.on('moderation', 'onSupportChangeStatus', (reportID: number, status: ModerationSupportStatus) => {
    ...
    React.useEffect(() => {
        return () => {
            Gateway.disconnect('moderation-selectSupport')
        }
    }, [])
    ...
    async function loadReport() {
        ...
        const result = await APISync({
            url: '/defaultapi/moderation/support/',
            type: 'get',
            data: {
                supportID
            }
        ...
        if(result.statusCode === 200) {
            setReport(result.message.support)
            setMessages(result.message.messages)

            setLoader(false)
            Gateway.init('moderation-selectSupport')
        }
        else {
            if(result.statusCode === 403) setErrorPage({ code: 403 })
            else setNavigate('/')

            notify("(reportcard) /moderation/support: " + result.message, { debug: true })
        }
    }
    ...
    return (
        <div className="supportcard" ref={globalRef}>
            ...
            <div className="reportHeader">
                <div className="reportTitle">
                    <h6>{Language("SUPPORT_NUMBER", null, null, report.id)}</h6>
                </div>
    ...
    function MessageAttachEntity({
        ...
        return (
            <div className="messageAttachEntity">
                {product ? (<AdCart product={product} size={"min"} type={"horizontal"} cartLink={"/products/"} />) : ''}
...
function FormBlock({
    ...
    async function onSubmit() {
        ...
        data.append('supportID', report.id.toString())
        ...
        const result = await APISync({
            url: '/defaultapi/moderation/support/message',
            type: 'post',
            data,
            contentType: false,
            processData: false
        ...
        if(result.statusCode === 200) {
            ...
            if(result.statusCode === 403) setErrorPage({ code: 403 })
                ...
                return setNavigate('/support')
            }
            else {
                notify("(account.support.id) /user/support/message: " + result.message, { debug: true })
...
function InfoModal({
    ...
    async function onChangeStatus() {
        ...
        const result = await APISync({
            url: '/defaultapi/moderation/support/status',
            type: 'put',
            data: {
                supportID: report.id
            }
        ...
        if(result.statusCode === 200) {
            Alert(Language("MODERATION_SUPPORT_CHANGE_STATUS", null, null, Language("REPORT_STATUS_NAME_" + result.message.toUpperCase())), 'success')
            setReport(old => {
            ...
            if(result.statusCode === 401) setErrorPage({ code: 401 })
                ...
                Alert(Language("MODERATION_REPORT_CHANGE_STATUS_ERROR"))
                ...
                body={(
                    <div className='reportInfoModalBody'>
                        ...
                        <div className="actions">
                            <Button name={report.status === 'closed' ? Language("OPEN") : Language("CLOSE")} size={"medium"} onClick={onChangeStatus} />
                        </div>
                    </div>
                )}
